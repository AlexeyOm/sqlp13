...
--мен€ем тип столбца, будет не адрес, а ссылка на адрес
alter table workers 
...
-- создадим таблицу - корзину, в которой будем хранить количество заказанных продуктов с прив€зкой к номеру заказа
...
--св€жем заказы и пользователей
alter table orders 
	add column customer_id int references customers(customer_id);

--св€жем заказы поставщикам и продукты, заказы и расходы, поправим таблицу заказов поставщикам
